...
//method to remove a word from a tree, recieves a node of the tree and the word to remove, returns true if the word existed and false otherwise
bool Trie::delT(TrieNode* curNode, string key)
{
	...
	bool flag=delT(curNode->children[CHAR_TO_INDEX(cur)], rest);
